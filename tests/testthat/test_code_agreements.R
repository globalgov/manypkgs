...
# Test that the punctuation marks are not in the treaty_ID
test_that("Punctation marks are not in the treaty_ID", {
  treaty_ID <- code_agreements(data5, data5$title, data5$date)
  expect_false(any(grepl("\\(|\\)", treaty_ID)))
})
