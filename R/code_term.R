...
code_term_type <- function(title, text = NULL){
  ...
  if (missing(title)) {
    ...
    type <- knitr::kable(type, "simple", caption = "Treaty Termination Type, source: https://www.srdlawnotes.com/2017/08/termination-of-treaties.html")
    type
  }  else {
  ...
  # word "For the Period"
  type <- ifelse(grepl("For The Period", title) & grepl("\\d{1,2}.\\w{3,}.\\d{4} To \\d{1,2}.\\w{3,}.\\d{4}", title, ignore.case = T), paste0("EXP"), NA)
  type
