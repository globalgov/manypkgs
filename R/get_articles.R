...
get_articles <- function(textvar, article = NULL,
  ...
  if (isTRUE(article == "preamble")) {
    ...
    grep("([^\\s]+\\s+){50}open for accession([^\\s]+\\s+){50}|
          |([^\\s]+\\s+){50}accession shall be([^\\s]+\\s+){50}|
          |([^\\s]+\\s+){50}accede to([^\\s]+\\s+){50}|
          |([^\\s]+\\s+){50}may join([^\\s]+\\s+){50}|
          |([^\\s]+\\s+){50}open for joining([^\\s]+\\s+){50}|
          |([^\\s]+\\s+){50}open for signature([^\\s]+\\s+){50}|
          |([^\\s]+\\s+){50}may become a member([^\\s]+\\s+){50}}|
          |([^\\s]+\\s+){50}accede thereto([^\\s]+\\s+){50}|
          |([^\\s]+\\s+){50}become parties([^\\s]+\\s+){50}|
          |([^\\s]+\\s+){50}become a party([^\\s]+\\s+){50}|
          |([^\\s]+\\s+){50}request accession([^\\s]+\\s+){50}|
          |([^\\s]+\\s+){50}may be admitted([^\\s]+\\s+){50}",
