...
standardise_date_input <- function(dates) {
  ...
  # makes NAs will not create errors
  dates <- stringr::str_remove_all(dates, "(0000-00-00|00-00-0000|00-00-00)")
  # standardising null dates
  dates <- stringr::str_replace_all(dates, "\\.", "-")
  # standardising separaters
  dates <- stringr::str_replace_all(dates, "\\/", "-")
  # standardising separaters
  dates <- stringr::str_replace_all(dates, "-([:digit:])-", "-0\\1-")
  # standardising months
  dates <- stringr::str_replace_all(dates, "-([:digit:])$", "-0\\1")
  # standardising days
  dates <- stringr::str_replace_all(dates, "^([:digit:])-", "0\\1-")
  # standardising days 2
  dates <- stringr::str_replace_all(dates, "-00|-\\?\\?|-NA", "")
  # standardising ambiguities
  dates <- stringr::str_replace_all(dates, "_", ":")
  # standardising ranges
...
correct_date_format <- function(dates) {
  ...
  incomp_dates <- function(dates) {
    ...
    x <- matrix(as.numeric(unlist(strsplit(dates, "-"))), ncol = 3, byrow = TRUE)
    ...
    out <- apply(x, 1, function(x) {
        y <- ifelse(nchar(x[ypos]) > 2,
                    ifelse(x[ypos] > thresh & x[ypos] != "9999",
                           ...
                           formatC(x[ypos], width = 4, flag = "0")),
                           # if 4, keep or correct
                    ifelse(as.numeric(x[ypos]) > (thresh %% 100),
                           paste0("19", formatC(x[ypos], width = 2, flag = "0")),
                           # if more than now, likely last century
                           paste0("20", formatC(x[ypos], width = 2, flag = "0"))))
                           # if less than now, likely this century
  ...
  dates <- ifelse(stringr::str_detect(dates, "^[:digit:]{2}-[:digit:]{2}-[:digit:]{4}$"),
                  as.character(as.Date(dates, "%d-%m-%Y")), dates)
  # Correct date order if need
  dates <- ifelse(stringr::str_detect(dates, "^[:digit:]{1}-[:digit:]{1}-[:digit:]{4}$"),
                  as.character(as.Date(dates, "%d-%m-%Y")), dates)
  # Correct date order and size
  dates <- ifelse(stringr::str_detect(dates, "^[:digit:]{2}-[:digit:]{1}-[:digit:]{4}$"),
                  as.character(as.Date(dates, "%d-%m-%Y")), dates)
  # Correct date order and size
  dates <- ifelse(stringr::str_detect(dates, "^[:digit:]{1}-[:digit:]{2}-[:digit:]{4}$"),
                  as.character(as.Date(dates, "%d-%m-%Y")), dates)
  # Correct date order and size
  dates <- ifelse(stringr::str_detect(dates, "^[:digit:]{3}-[:digit:]{2}-[:digit:]{2}$"),
                  paste0("0", dates), dates)
  # correct year size if missing 0 before year
  dates <- ifelse(stringr::str_detect(dates, "^[:alpha:]{3}\\s[:digit:]{2}\\,\\s[:digit:]{4}$"),
                  as.character(as.Date(dates, "%b %d, %Y")), dates)
  # Correct format
  dates <- ifelse(stringr::str_detect(dates, "^[:alpha:]{3}\\s[:digit:]{1}\\,\\s[:digit:]{4}$"),
                  as.character(as.Date(dates, "%b %d, %Y")), dates)
  # Correct format
  dates <- ifelse(stringr::str_detect(dates, "^[:digit:]{1}-[:digit:]{1}-[:digit:]{2}$"),
                  incomp_dates(dates), dates)
  # for incomplete dates with 4 digits only
  dates <- ifelse(stringr::str_detect(dates, "^[:digit:]{2}-[:digit:]{1}-[:digit:]{2}$"),
                  incomp_dates(dates), dates)
  # for incomplete dates with 5 digits only
  ...
  dates <- ifelse(stringr::str_detect(dates, "^[:digit:]{2}-[:digit:]{2}-[:digit:]{2}$"),
                  incomp_dates(dates), dates)
  # for incomplete dates with 6 digits only
  dates <- ifelse(stringr::str_detect(dates, "0000-00-00"), NA, dates)
}
...
treat_historical_dates <- function(dates) {
  ...
  neg_dates <- function(dates) {
    if (stringr::str_detect(dates, "^-[:digit:]{4}$")) {
      # negative dates with 4 digit year only
      ...
      histdate <- as.numeric(lubridate::as_date(negdate) + dt)
      dates <- lubridate::as_date(histdate)
    } else if (stringr::str_detect(dates, "^-[:digit:]{3}$")) {
      # negative dates with 3 digit year only
      ...
      histdate <- as.numeric(lubridate::as_date(negdate) + dt)
      dates <- lubridate::as_date(histdate)
    } else if (stringr::str_detect(dates, "^-[:digit:]{2}$")) {
      # negative dates with 2 digit year only
  ...
  dates <- ifelse(stringr::str_detect(dates, "^-[:digit:]{4}-[:digit:]{2}-[:digit:]{2}$"),
                  neg_dates_comp(dates), dates) # negative ymd dates
  dates <- ifelse(stringr::str_detect(dates, "^-[:digit:]{2}-[:digit:]{2}-[:digit:]{4}$"),
                  neg_dates_comp(dates), dates) # negative dates in dmy format
  dates <- ifelse(stringr::str_detect(dates, "^-[:digit:]{2}$"),
                  neg_dates(dates), dates) # negative 2 years only dates
  dates <- ifelse(stringr::str_detect(dates, "^-[:digit:]{3}$"),
                  neg_dates(dates), dates) # negative 3 years only dates
  dates <- ifelse(stringr::str_detect(dates, "^-[:digit:]{4}$"),
                  neg_dates(dates), dates) # negative year only dates
}
...
treat_future_dates <- function(dates) {
  ...
  dates <- ifelse(stringr::str_detect(dates, "^[:digit:]{4}-[:digit:]{2}-[:digit:]{2}$"),
                  fut_dates(dates), dates) # stadardises how future dates are reported
}
...
treat_incomplete_dates <- function(dates) {
  ...
  dates <- sapply(dates, function(d) {
    if (is.na(d)) {
      d <- d
    } else if (stringr::str_detect(d, "^[:digit:]{4}$")) {
      # 4 digit year only
      d <- paste0(d, "-01-01:", d, "-12-31")
      d
    } else if (stringr::str_detect(d, "^[:digit:]{3}$")) {
      # 3 digit year only
      d <- paste0("0", d, "-01-01:", "0", d, "-12-31")
      d
    } else if (stringr::str_detect(d, "^[:digit:]{2}$")) {
      # 2 digit year only
      d <- paste0("00", d, "-01-01:", "00", d, "-12-31")
      d
    } else if (stringr::str_detect(d, "^[:digit:]{4}-[:digit:]{2}$")) {
      # month only
      start <- paste0(d, "-01")
  ...
  dates <- lapply(dates, function(d) {
    ...
    if (is.na(d)) {
      ...
      finish <- paste(stringr::str_split(start, "-")[[1]][1],
                      ...
                      sep = "-")
      d <- date_range(start, finish)
    } else if (stringr::str_detect(d, "^[:digit:]{4}-[:digit:]{2}-[:digit:]{2}:[:digit:]{2}$")) {
      # day range
      brackets <- stringr::str_split(d, ":")
      start <- brackets[[1]][1]
      finish <- paste(stringr::str_split(start, "-")[[1]][1],
