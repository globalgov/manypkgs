...
retain <- function(keep = NULL, envir = .GlobalEnv, keep_functions = TRUE,
  ...
  #----[ processing ]----#
  {
    # if user wants to automatically detect
    # regex or manually force it
    if (is.element(regex, c(TRUE, "auto", 1))) {
      ...
      # if user manually force the code not to
      # concider regulat expression
    } else {
      regex <- FALSE
    }

    # in case user decides to only operates
    # on non-function variables
    if (keep_functions) {
    ...
    #----[ check if user inserted a variable name or
    # regexp that does not exist ]----#
    {
