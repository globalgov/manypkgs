...
Using `{manydata}`, we can download and check which datacubes and datasets are available in the packages.
...
Different treatyIDs generated with `code_agreements()` for different datasets in a datacube
...
The 'many' packages contain datacubes that store the original texts of treaties,
where available digitally,
listed in the `agreements`, `memberships`, and `references` datacubes.
