...
in the `manyenviron::agreements` database,
...
If we randomly create a variable and use it to find duplicates, the AUC scores might differ marginally. 
...
Let's actually only consider equivalent those titles that are exactly
...
Fuzzy matching relies on `stringdist` to match observations that
...
agreements$treatyID_strict <- ifelse(duplicated(agreements$treatyID) |
                                  duplicated(agreements$treatyID,
                                             fromLast = TRUE), 1, 0)
WVPlots::ROCPlot(agreements, "treatyID_strict", "duplicate", 1,
...
We can also improve the AUC scores by fuzzy matching the new treatyIDs
generated from different datasets with the `condense_agreements()` function.
...
agreements$manyID_sim <- ifelse(duplicated(agreements$manyID) |
                                  duplicated(agreements$manyID,
                                             fromLast = TRUE), 1, 0)
WVPlots::ROCPlot(agreements, "manyID_sim", "duplicate", 1, title = "treatyID Sim")
...
We can compare the specificity versus sensitivity
...
WVPlots::ROCPlotList(frame = agreements,
                     xvar_names = c("strict", "fuzzy", "treatyID_strict", "manyID_sim"),
