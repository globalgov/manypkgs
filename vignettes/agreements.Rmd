...
library(qCreate)
samples <- lapply(qEnviron::agreements, function(x) x[x$Beg > "1990-01-01" & x$Beg < "2000-01-01", ])
...
sample$language <- cld2::detect_language(sample$titles)
sample <- sample %>% 
  dplyr::filter(language == "en")

small_sample <- dplyr::sample_n(sample, 400)
small_sample <- small_sample %>% 
  dplyr::arrange(dates) %>% 
  dplyr::select(titles, dates)

# We hand coded 400 agreements from the sample to compare duplicates identification.
library(readr)
agreements <- readr::read_delim("hand-coded-sample-2.csv", ";", escape_double = FALSE, trim_ws = TRUE)
agreements$dates <- lubridate::dmy(agreements$dates)
```
...
agreements$unique <- 0
WVPlots::ROCPlot(frame = agreements, xvar = unique, truthVar = duplicate, truthTarget = 1, title = "Unique")
...
### Condensing qID matches

We can also improve the AUC scores by fuzzy matching the new qIDs generated by `code_agreements()` from different
datasets with the `condense_qID()`.

```{r qsim, warning=FALSE, message=FALSE}
qsim <- condense_qID(var = agreements$qID)
agreements <- dplyr::left_join(agreements, qsim, by = "qID")
agreements$qsim <- ifelse(duplicated(agreements$qID_ref) |
                            duplicated(agreements$qID_ref, fromLast = TRUE), 1, 0)
