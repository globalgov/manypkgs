...
## Contributing to the qData ecosystem of packages with manypkgs
...
```{r eval=FALSE}
manypkgs::setup_package("qStates", name = "Hollway, James")
```
...
```{r eval=FALSE}
manypkgs::update_package("qStates")
```
...
```{r eval=FALSE}
COW <- manypkgs::transmutate(ID = stateabb,
                   Beg = manypkgs::standardise_dates(styear, stmonth, stday),
                   End = manypkgs::standardise_dates(endyear, endmonth, endday),
...
Once the dataset has been corrected in `{qData}`, the cleaned dataset can be loaded with `{manypkgs}` into the 
